...
export function Text({ className, ...props }: React.ComponentPropsWithoutRef<'p'>) {
  return <p className={cn('text-base/6 text-zinc-500 dark:text-zinc-400', className)} {...props} />
}
        ...
        <div className="grid min-h-full grid-rows-[1fr_auto] justify-items-center sm:grid-rows-[1fr_auto_3fr] sm:p-4">
          <Headless.DialogPanel
            ...
            className={cn(
              ...
              prose ? 'prose prose-zinc dark:prose-invert' : ''
            )}
          >
            {children}
          </Headless.DialogPanel>
...
export function DialogDescription({
  className,
  ...props
}: { className?: string } & Omit<React.ComponentPropsWithoutRef<typeof Text>, 'className'>) {
