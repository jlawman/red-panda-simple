...
export default function LogoGenerationPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/fal-flux-1-1-pro', {
        ...
        body: JSON.stringify({
          prompt: `Generate a logo based on this description: ${prompt}. The logo should be ${aspectRatio === 'custom_900x300' ? '900x300' : aspectRatio}-shaped.`,
      ...
      if (result.data && result.data.images && Array.isArray(result.data.images)) {
        setGeneratedLogos(result.data.images.map(img => img.url || img));
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      <Heading className="text-center mb-8">Logo Generator</Heading>
