...
6. Brainstorm business ideas:
   a. Generate at least 6 potential SAAS business ideas that leverage generative AI (or traditional ML) to address the identified pain points
   b. Briefly describe each idea and its potential impact

7. Evaluate and refine the best 3 ideas:
   a. Select the most promising 3 ideas from your brainstorming session
...
Present your overall analysis in <analysis></analysis> tags and 3 final business ideas within <business_idea><business_name></business_name><business_description></business_description></business_idea> tags one business tags pair for each idea, using appropriate subheadings for each section. Ensure your ideas are innovative, feasible, and directly addresses the pain points identified in the given business vertical while leveraging recent generative AI.`
  ...
  try {
    ...
    const businessIdeaMatches = result.matchAll(/<business_idea>(.*?)<\/business_idea>/gs);
    const businessIdeas = Array.from(businessIdeaMatches).map(match => {
      ...
      const nameMatch = ideaText.match(/<business_name>(.*?)<\/business_name>/s);
      const descriptionMatch = ideaText.match(/<business_description>(.*?)<\/business_description>/s);
